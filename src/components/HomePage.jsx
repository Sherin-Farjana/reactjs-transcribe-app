...
export default function HomePage(props) {
    ...
    // State for recording status, audio chunks, and recording duration
    ...
    async function stopRecording() {
        ...
        // On stop, combine audio chunks into a Blob
    ...
    // Track recording duration in seconds
    useEffect(() => {
    ...
    return (
        <main className='flex-1  p-4 flex flex-col gap-3 text-center sm:gap-4  justify-center pb-20'>
            {/* App title */}
            <h1 className='font-semibold text-5xl sm:text-6xl md:text-7xl'>Deep<span className='text-blue-400 bold'>Scribe</span></h1>
            ...
            <button onClick={recordingStatus === 'recording' ? stopRecording : startRecording} className='flex specialBtn px-4 py-2 rounded-xl items-center text-base justify-between gap-4 mx-auto w-72 max-w-full my-4'>
                ...
                {/* Duration and microphone icon */}
                <div className='flex items-center gap-2'>
                    {duration !== 0 && (
                        <p className='text-sm text-black'>{duration}s</p>
                    )}
                    <i className={"fa-solid duration-200 text-blue-400 fa-microphone " + (recordingStatus === 'recording' ? ' text-rose-300' : "")}></i>
                </div>
            </button>
